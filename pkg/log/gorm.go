...
func (lg *GormLogger) Info(ctx context.Context, msg string, data ...interface{}) {
	if lg.LogLevel >= logger.Info {
		...
		lg.logger.Infof(tpl, append([]interface{}{utils.FileWithLineNum()}, data...)...)
	}
}
...
func (lg *GormLogger) Warn(ctx context.Context, msg string, data ...interface{}) {
	if lg.LogLevel >= logger.Warn {
		...
		lg.logger.Warnf(tpl, append([]interface{}{utils.FileWithLineNum()}, data...)...)
	}
}
...
func (lg *GormLogger) Error(ctx context.Context, msg string, data ...interface{}) {
	if lg.LogLevel >= logger.Error {
		...
		lg.logger.Errorf(tpl, append([]interface{}{utils.FileWithLineNum()}, data...)...)
	}
}
...
func (lg *GormLogger) Trace(ctx context.Context, begin time.Time, fc func() (sql string, rowsAffected int64), err error) {
	...
	case err != nil && lg.LogLevel >= logger.Error && (!errors.Is(err, logger.ErrRecordNotFound) || !lg.IgnoreRecordNotFoundError):
		sql, rows := fc()
		if rows == -1 {
			lg.logger.Errorf(traceErrStr, utils.FileWithLineNum(), err, float64(elapsed.Nanoseconds())/1e6, "-", sql)
		} else {
			lg.logger.Errorf(traceErrStr, utils.FileWithLineNum(), err, float64(elapsed.Nanoseconds())/1e6, rows, sql)
		}
	case elapsed > lg.SlowThreshold && lg.SlowThreshold != 0 && lg.LogLevel >= logger.Warn:
		sql, rows := fc()
		slowLog := fmt.Sprintf("SLOW SQL >= %v", lg.SlowThreshold)
		if rows == -1 {
			lg.logger.Warnf(traceWarnStr, utils.FileWithLineNum(), slowLog, float64(elapsed.Nanoseconds())/1e6, "-", sql)
		} else {
			lg.logger.Warnf(traceWarnStr, utils.FileWithLineNum(), slowLog, float64(elapsed.Nanoseconds())/1e6, rows, sql)
		}
	case lg.LogLevel == logger.Info:
		sql, rows := fc()
		tpl := genLogTpl(ctx, traceStr, nil)
		if rows == -1 {
			lg.logger.Infof(tpl, utils.FileWithLineNum(), float64(elapsed.Nanoseconds())/1e6, "-", sql)
		} else {
			lg.logger.Infof(tpl, utils.FileWithLineNum(), float64(elapsed.Nanoseconds())/1e6, rows, sql)
		}
	}
}
